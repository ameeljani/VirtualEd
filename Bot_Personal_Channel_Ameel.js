...
app.get('/privacy', (req, res) => {
  res.send('The Unsplash Chatbot for Zoom does not store any user data, according to Zoom itself.')
})
...
app.post('/unsplash', (req, res) => {
  ...
  if(count==0)
  {
     ...
     {
        errorstart='There are no current questions to vote on'
        getChatbotToken2()
     }
    
     else
     {
      if((req.payload.body.cmd).charAt((req.payload.body.cmd).length-1)=='?')
      {
     ...
     replytype='Your typed question has been recorded'
    ...
    {
      errorstart='There are no current questions to vote on and this is not a question'
      getChatbotToken2()
    }
    }
  }
  else
  {
    ...
    else
    {
      ...
      var isquestion=questioncheck()
      
      if(isquestion==false)
      {
        errorstart='The text you entered is not a question'
        getChatbotToken2()
      }
      else
      {
    ...
    request({
      ...
      body: {
        ...
        'content': {
          ...
          'body': {
            'type': 'message',
            'text': errorstart+', please type one below'
